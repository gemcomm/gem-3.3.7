...
if [ -z "${execdir}" ] ; then
  echo "Um_cleanup.ksh requires an '-execdir' argument for cleanup path"
  exit 1
fi
if [ ! -d ${execdir} ] ; then
  echo "Um_cleanup.ksh encountered an invalid execdir: ${execdir}"
  exit 1
fi
...
if [ ${transfer} -gt 0 -a -d ${xf_taskdir} ] ; then
  echo "No cleanup for transfer component"
fi
